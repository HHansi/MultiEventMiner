    ...
    def load(cls, pretrained_model_name_or_path, revision=None, n_added_tokens=0, language_model_class=None, **kwargs):
        ...
        * facebook/dpr-ctx_encoder-single-nq-base
        ...
        if os.path.exists(config_file):
            ...
            # it's a local directory in FARM2 format
    ...
    def get_language_model_class(model_name_or_path, **kwargs):
        ...
        if model_type == "xlm-roberta":
            ...
            if "mlm" in model_name_or_path.lower():
                raise NotImplementedError("MLM part of codebert is currently not supported in FARM2")
...
class Bert(LanguageModel):
    ...
    def load(cls, pretrained_model_name_or_path, language=None, **kwargs):
        ...
        * OR a local path of a model trained via FARM2 ("some_dir/farm_model")
        ...
        # We need to differentiate between loading model using FARM2 format and Pytorch-Transformers format
    ...
    def load(cls, pretrained_model_name_or_path, language=None, **kwargs):
        ...
        * or a local path of a model trained via FARM2 ("some_dir/farm_model")
        ...
        :param language: (Optional) Name of language the model was trained for (e.g. "german").
                         If not supplied, FARM2 will try to infer it from the model name.
...
class EmbeddingModel():
    ...
    def resize_token_embeddings(self, new_num_tokens=None):
        # function is called as a vocab length validation inside FARM2
...
class WordEmbedding_LM(LanguageModel):
    ...
    - Inside FARM2, WordEmbedding Language Models must have a fixed vocabulary
    ...
    def load(cls, pretrained_model_name_or_path, language=None, **kwargs):
        ...
        * a local path of a model trained via FARM2 ("some_dir/farm_model")
...
class Electra(LanguageModel):
    ...
    def load(cls, pretrained_model_name_or_path, language=None, **kwargs):
        ...
        * OR a local path of a model trained via FARM2 ("some_dir/farm_model")
        ...
        # We need to differentiate between loading model using FARM2 format and Transformers format
        farm_lm_config = Path(pretrained_model_name_or_path) / "language_model_config.json"
        if os.path.exists(farm_lm_config):
            # FARM2 style

...
class TextClassificationHead(PredictionHead):
    ...
    def load(cls, pretrained_model_name_or_path, revision=None, **kwargs):
        """
        Load a prediction head from a saved FARM2 or transformers model. `pretrained_model_name_or_path`
        can be one of the following:
        a) Local path to a FARM2 prediction head config (e.g. my-bert/prediction_head_0_config.json)
    ...
    def load(cls, pretrained_model_name_or_path, revision=None, n_added_tokens=0, **kwargs):
        ...
        if os.path.exists(pretrained_model_name_or_path) \
            ...
            if n_added_tokens != 0:
                ...
                raise NotImplementedError("Custom vocab not yet supported for model loading from FARM2 files")

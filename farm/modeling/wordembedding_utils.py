...
class Fasttext_converter():
    """
    Class to use fasttext inside FARM2 by converting embeddings to format usable by preprocessing pipeline.
...
def convert_WordEmbeddings(embedding_filename, vocab_filename, output_path, language="English"):
    """
    Converts a Wordembedding model in word2vec format to a format that can be used inside FARM2
...
def fit_s3e_on_corpus(processor, model, corpus, n_clusters=10,
    ...
    :param processor: FARM2 Processor with a Tokenizer used for reading the corpus (e.g. Inference Processor)
    :param model: FARM2 AdaptiveModel with an embedding layer in the LM (currently only supporting 'WordEmbedding_LM' as a language model)

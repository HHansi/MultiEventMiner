...
class AdaptiveModel(nn.Module, BaseAdaptiveModel):
    ...
    def load(cls, load_dir, device, strict=True, lm_name=None, processor=None):
        ...
        * vocab.txt vocab file for language model, turning text to Wordpiece Tokens
        ...
        :param strict: whether to strictly enforce that the keys loaded from saved model match the ones in
                       ...
                       Set to `False` for backwards compatibility with PHs saved with older version of FARM2.
        :type strict: bool
    ...
    def convert_to_onnx(cls, model_name, output_path, task_type, convert_to_float16=False, quantize=False, opset_version=11):
        ...
        # save processor & model config files that are needed when loading the model with the FARM2 Inferencer
...
class ONNXAdaptiveModel(BaseAdaptiveModel):
    ...
    Existing PyTorch based FARM2 AdaptiveModel can be converted to ONNX format using AdaptiveModel.convert_to_onnx().
    The conversion is currently only implemented for Question Answering Models.

    For inference, this class is compatible with the FARM2 Inferencer.
    """

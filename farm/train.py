...
class Trainer:
    ...
    def __init__(
        ...
        self.eval_report_df = pd.DataFrame()
        self.train_progress_file = train_progress_file  # .csv file
    ...
    def train(self):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                if self.evaluate_every != 0 \
                    ...
                    if dev_data_loader is not None:
                        ...
                        # save train progress
                        for head_num, head in enumerate(result):
                            head = dict(collections.OrderedDict(sorted(head.items())))
                            if 'report' in head.keys():
                                del head['report']
                            self.eval_report_df = self.eval_report_df.append(head, ignore_index=True)
                        if self.train_progress_file is not None:
                            self.eval_report_df.to_csv(self.train_progress_file, index=False)
        ...
        if self.early_stopping and self.early_stopping.save_dir:
            ...
            # Repeat evaluation on Dev set using the best model
            dev_data_loader = self.data_silo.get_data_loader("dev")
            if dev_data_loader is not None:
                evaluator_dev = Evaluator(
    ...
    def _save(self):
        ...
        bound to specifics Python version, FARM2 version, directory structures etc. A more modular and reusable approach

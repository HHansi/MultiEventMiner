...
class Inferencer:
        ...
        if task_type == "embeddings":
            if not extraction_layer or not extraction_strategy:
                    logger.warning("Using task_type='embeddings', but couldn't find one of the args `extraction_layer` and `extraction_strategy`. "
                                   "Since FARM2 0.4.2, you set both when initializing the Inferencer and then call inferencer.inference_from_dicts() instead of inferencer.extract_vectors()")
    ...
    def load(
        ...
        **kwargs
        ...
        :param use_fast: (Optional, True by default) Indicate if FARM2 should try to load the fast version of the tokenizer (True) or
    ...
    def inference_from_dicts(
        ...
        * QA (FARM2 style): [{"questions": ["What is X?"], "text":  "Some context containing the answer"}]
